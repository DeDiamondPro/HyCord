...
public class VoiceCreator extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        mc.fontRendererObj.drawStringWithShadow("Lock voice call:", 4, 16, new Color(255, 255, 255).getRGB());
        Gui.drawRect(83, 15, 93, 25, new Color(255, 255, 255).getRGB());
        Gui.drawRect(84, 16, 92, 24, new Color(0, 0, 0).getRGB());
        if (locked) {
            Gui.drawRect(85, 17, 91, 23, new Color(255, 0, 0).getRGB());
        }

        mc.fontRendererObj.drawStringWithShadow("Capacity: " + capacity, 4, 28, new Color(255, 255, 255).getRGB());
        Gui.drawRect(70, 29, 270, 34, new Color(50, 50, 50).getRGB());
        Gui.drawRect((int) Utils.map(capacity, 1, 100, 70, 270), 27, (int) Utils.map(capacity, 1, 100, 73, 273), 36, new Color(200, 200, 200).getRGB());

        int length = 6;
        int height = 57;
        mc.fontRendererObj.drawStringWithShadow("Game:", 4, 40, new Color(255, 255, 255).getRGB());
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) {
        if (mouseX >= this.width / 2 - 50 && mouseX <= this.width / 2 + 50 && mouseY >= this.height - 30 && mouseY <= this.height - 10) {
            LobbyManager.createVoice(capacity, LobbyType.PUBLIC, selectedGame, selectedTopic, locked);
            ...
            locked = !locked;
        } else if (mouseX >= 70 && mouseX <= 270 && mouseY >= 27 && mouseY <= 36) {
            editing = true;
